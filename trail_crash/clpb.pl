...
satisfiable_bdd(BDD) :-
        ...
        ;   bdd_nodes(BDD, Nodes),
            (   maplist(node_var_is_var, Nodes, Vs0) ->
                ...
                aliasing_consistency(BDD, VNs, Goals),
...
aliasing_consistency(BDD, VNs, Goals) :-
        include(always_branching, VNs, Bs0),
        pairs_keys(Bs0, Bs1),
        variables_in_index_order(Bs1, Bs),
...
always_branching(_-Nodes) :- maplist(node_always_branching, Nodes).

node_always_branching(Node) :-
        node_var_low_high(Node, _, Low, High),
        Low \== 1,
        High \== 1.

always_decisive(_-Nodes) :- maplist(node_always_decisive, Nodes).

node_always_decisive(Node) :-
        node_var_low_high(Node, _, Low, High),
        (   Low == 0 -> true
        ;   High == 0 -> true
        ;   false
